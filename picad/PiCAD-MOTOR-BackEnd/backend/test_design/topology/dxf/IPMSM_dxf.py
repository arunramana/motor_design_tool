...
#GenerateDXF is a class that draws the dxf of a motor (IPMSM for now)
#uses ezdxf library
...
class GenerateDXF_IPMSM:
    ...
    def draw_motor(self):
        ...
        with r12writer(self.dxf_file) as doc:
           ...
           # doc.add_circle((xo,yo), radius=stator_id)
            ...
            if(not self.has_notch):
                doc.add_circle((self.xo,self.yo), radius=self.rotor_or)
        ...
        #draw entire motor for fd diagram
        motor_filename = self.directory+"/motor_full.dxf"
        with r12writer(motor_filename) as doc:
           ...
           # doc.add_circle((xo,yo), radius=stator_id)
            ...
            #draw stator
            self.draw_stator(doc,False)

            #draw rotor
            self.draw_rotor(doc)
            
        if(not os.path.isdir(self.directory+"/download")):
            
            os.mkdir(self.directory+"/download")

        # stator for download
        stator_filename = self.directory+"/download/stator.dxf"
        with r12writer(stator_filename) as doc:
        ...
        rotor_filename = self.directory+"/download/rotor.dxf"
        with r12writer(rotor_filename) as doc:
    ...
    def draw_rotor(self,doc):
        ...
        for iter_no in range(0,self.no_poles):
            ...
            r2 = self.draw_pole(doc,self.rotor_or,self.pole_length,self.pole_width,self.pole_dist,self.pole_angle,reference_angle)
            ...
            for i in range(len(self.duct_distances)):
                ...
                self.draw_rotor_ducts(doc,reference_angle,duct_dist,duct_angle,duct_radius,self.pole_dist,r2,iter_no)


            if(self.has_notch):
                coords = self.draw_rotor_notch(doc,reference_angle,self.no_poles,self.has_notch,self.notch_depth,self.notch_angle,self.rotor_or)

                notch_coord.append(coords)


        if(self.has_notch):

            #connect all the notches

            #l        r
            r0,theta0,r1,theta1 = notch_coord[0]



            for i in range(1,len(notch_coord)):

                r2,theta2,r3,theta3 = notch_coord[i]

                doc.add_arc((self.xo,self.yo),radius=r0, start=90-theta2, end=90-theta1)

                r1,theta1=r3,theta3


            doc.add_arc((self.xo,self.yo),radius=r0, start=90-theta0, end=90-theta1)

    def draw_pole(self,doc,rotor_or,pole_length,pole_width,pole_distance,pole_angle,reference_angle):

        #draws the pole(magnet) of the rotor
        theta = pole_angle/2

        r1 = pole_length/(sin(theta)*2)
        ...
        if(rotor_or <= h):
            ...
            print("Error: Change pole dimensions to prevent pole from going out of bounds")
            return


        theta_t = taninv(pole_length/(2*h))

        r2 = (pole_length)/(2*sin(theta_t))


        if(pole_distance >= rotor_or or r2>rotor_or):

            print("Error: Change pole dimensions or bridge thickness to prevent pole from going out of bounds")
            return
    ...
    def draw_winding(self,doc,no_slots,no_poles,theta_inc_stator,stator_ir,stator_or,yoke_thickness,tooth_width):
        ...
        winding_pattern = GenerateDXF_IPMSM.getWindingPattern(no_slots,no_poles)
        ...
        while i < len(coords):
            ...
            winding = winding_pattern[i1]
            incircuit = GenerateDXF_IPMSM.getCircuitLabels(winding)


            doc.add_point((x1,y1))
            doc.add_text(incircuit,(x1,y1-2),height=2,width=2)
            
            winding = winding_pattern[i2]
            incircuit = GenerateDXF_IPMSM.getCircuitLabels(winding)
