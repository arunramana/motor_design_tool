...
class fnc_widget (QWidget):
    ...
    def __init__(self, parent=None):
           ...
           self.chart_view.connect(self.update_space_vector)
    ...
    def add_series(self,t,a,b,c):
       """
       Add the abc phase quantites versus time t and generated the related
       space-vector sv.
       It is also available the add_sv() function wich generates the
       abc quantites starting from a space-vector
       """
    ...
    def add_sv(self,t,alpha,beta):
       """
       Generate the abc quantities starting from the alpha-beta components
       of a space vector.
       This function is almost the inverse of add_series().
       """


       # generate the abc quantities trajectory
       a = alpha
       b = -alpha/2 + np.sqrt(3)/2*beta
       c = -alpha/2 - np.sqrt(3)/2*beta
       self.chart_view.add_series(t,a,b,c)
       self.chart_view.chart().createDefaultAxes()

       trajectory =  QPainterPath();
       trajectory.moveTo(alpha[0],-beta[0])

       for id, a in enumerate(alpha[1:]):
       # for p in alpha[1:]:
          trajectory.lineTo(a,-beta[id])
    ...
    @Slot(float,float,float)
    def add_sv_dq(self,t,d,q):
       """
       Generate the trajectory and the d-q quantities related to
       a space vector in the d-q reference frame.
       """

       self.chart_view.add_series_dq(t,d,q)
       self.chart_view.chart().createDefaultAxes()

       trajectory =  QPainterPath();
       trajectory.moveTo(d[0],-q[0])

       for id, a in enumerate(d[1:]):
       # for p in alpha[1:]:
          trajectory.lineTo(a,-q[id])
       ...
       self.chart_view..disconnect(self.update_space_vector)
       self.chart_view.time_changed.connect(self.update_space_vector_dq)
    ...
    @Slot()
    def update_space_vector_dq(self,t,a,b,c):
        """
        A special version of the previous function to handle correct
        d-q space vector rapresentation (the time quantities are the
        d-q components, not the abc quantities)
        """
        ...
        msg = '(t:{:-8.3f}; d: {:-8.3f}; q: {:-8.3f};'.format(t,a,b)
        self.status_bar_time.setText('<pre>' + msg + '</pre>')
...
class fnc_chart_view (QChartView):
    ...
    def add_series_dq(self,t,y1,y2):
           ...
           line2.setName("q");
           #line.setPen(QPen(QColor("black"), 2))
           for _t, _y1, _y2 in np.nditer([t,y1,y2]):
                line1.append(_t, _y1)
                line2.append(_t, _y2)
           #line.hovered.connect(on_line_hover)
           self.chart().addSeries(line1)
           ...
           self.max_y = np.max([np.max(y1),np.max(y2)])
           self.min_y = np.min([np.min(y1),np.min(y2)])
           self.max_t = np.max(t)
           self.min_t = np.min(t)
           ...
           self.y1 = y1
           self.y2 = y2
           self.y3 = np.zeros(len(t)) # this is dummy in this case
